...
{
	char *p = (char *)malloc(strlen(s) + 1);//(char *)malloc(10*sizeof(char))
	if (p) strcpy(p, s);
	return p;
};

int e_vazia(nodeArvore *a);

void imprime_pre_order(nodeArvore *a);
void imprime_textual(nodeArvore *a);
...
nodeLista *novoNodeLista(nodeArvore *nArv)
{
	// Aloca memória
	nodeLista *novo;
	if ((novo = (nodeLista*)malloc(sizeof(nodeLista))) == NULL) return NULL;
...
nodeArvore *novoNodeArvore(byte c, int frequencia, nodeArvore *esquerda, nodeArvore *direita)
{
	...
	if ((novo = (nodeArvore *)malloc(sizeof(nodeArvore))) == NULL) return NULL;
...
void CompressFile(const char *arquivoEntrada, const char *arquivoSaida)
{
	...
	***/
	...
	double tamanhoSaida = ftell(saida) - sizeof(listaBytes) - sizeof(unsigned);
	...
	printf("Taxa de compressao: %.2f%%\n", 100-(float)((100 * tamanhoSaida) / tamanhoEntrada));
...
void DecompressFile(const char *arquivoEntrada, const char *arquivoSaida)
{
	...
	fseek(entrada, 0L, SEEK_END);
	double tamanhoEntrada = ftell(entrada) - sizeof(listaBytes) - sizeof(unsigned);
...
int main()
{
	...
	CompressFile("teste.txt", "testecomprimido.dildokuku");
	printf("O arquivo resultante da compressao deve ter a extensao '.hx'.\n");
	printf("Exemplo: \n\t./huffman -c comprima.isso nisso.hx\n");
	DecompressFile("testecomprimido.dildokuku", "testedescomprimido.txt");
...
void imprime_pre_order(nodeArvore *a) {
	if (!e_vazia(a)) {
		...
		imprime_pre_order(a->esquerda);
		imprime_pre_order(a->direita);
	}
}
...
void imprime_textual(nodeArvore *a) {
	if (e_vazia(a))
		...
		printf("<%c/%d", a->c, a->frequencia);
		imprime_textual(a->esquerda);
		imprime_textual(a->direita);
		printf(">");
	}
}